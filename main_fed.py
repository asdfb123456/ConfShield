...
from utils.defense import fltrust, multi_krum, get_update, RLR, flame, get_update2, fld_distance, detection, detection1, parameters_dict_to_vector_flt, lbfgs_torch, layer_krum, flare,DefenseMethods
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        dataset_train = datasets.CIFAR10(
            './data/cifar10', train=True, download=False, transform=trans_cifar)
        dataset_test = datasets.CIFAR10(
            './data/cifar10', train=False, download=False, transform=trans_cifar)
        if args.iid:
    ...
    baseline=copy.deepcopy(net_glob)
    fed_model_path = './model/' + args.load_fed + '.pth'
    baseline.load_state_dict(torch.load(fed_model_path))
    # training
    loss_train = []
    ...
    for iter in range(args.epochs):
        ...
        for num_turn, idx in enumerate(idxs_users):
            ...
            # if args.defence == 'ours':
            #     local_models.append(w_no_dict)
        ...
        elif args.defence == 'ours':
            Agg=DefenseMethods(args, dataset_test,iter)          
            local_models=Agg.Adaptive_Agg(w_locals,baseline)
            local_weights=[]
            for i in range(len(local_models)):
                local_weights.append(local_models[i].state_dict())
            scores=[1 for i in range(len(local_weights))]
            # #temp=temp-1
            #  # 20个参与训练的用户中选择
            # scores=AGG(local_models,net_glob,idxs_users)
            w_glob = Agg.average_weights(local_weights, scores)
    ...
    torch.save(net_glob.state_dict(), './model' + '/'+args.dataset +'.pth')
