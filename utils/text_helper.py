...
class TextHelper(Helper):
    ...
    def load_data(self):
        ...
        if self.params['size_of_secret_dataset'] % (self.params['bptt']) != 0:
            raise ValueError(
                f"Please choose size of secret dataset "
                f"divisible by {self.params['bptt'] }"
            )
        # print('text_helper.py line 102 self.params[word_dictionary_path]:',self.params['word_dictionary_path'])
        torch.load('data/reddit/50k_word_dictionary.pt')
        dictionary = torch.load(self.params['word_dictionary_path'])
        corpus_file_name = (
            f"{self.params['data_folder']}/"
            f"corpus_{self.params['number_of_total_participants']}.pt.tar"
        )
        if self.params['recreate_dataset']:
            self.corpus = Corpus(
                self.params, dictionary=dictionary, is_poison=self.params['is_poison']
            )
        ...
        if self.params['is_poison']:
            self.params['adversary_list'] = [POISONED_PARTICIPANT_POS] + random.sample(
                range(self.params['number_of_total_participants']),
                self.params['number_of_adversaries'] - 1,
            )
            logger.info(
                f"Poisoned following participants: {len(self.params['adversary_list'])}"
            )
        else:
        ...
        self.train_data = [
            self.batchify(data_chunk, self.params['batch_size'])
            for data_chunk in self.corpus.train
        ]
        # print('text_helper.py line132(len(self.train_data)):', self.train_data[0].shape)
        # print('text_helper.py line133(exit)')
        # exit()
        ...
        if self.params['is_poison']:
            ...
            self.poisoned_data = self.batchify(
                self.corpus.load_poison_data(
                    number_of_words=self.params['size_of_secret_dataset']
                    * self.params['batch_size']
                ),
                self.params['batch_size'],
            )  # 拿到即将被放毒的数据，此时并未放毒
            self.poisoned_data_for_train = self.poison_dataset(
                self.poisoned_data, dictionary, poisoning_prob=self.params['poisoning']
            )  # poisoning=1.0，全放毒
    ...
    def create_model(self):
        local_model = RNNModel(
            ...
            rnn_type='LSTM',
            ntoken=self.n_tokens,
            ninp=self.params['emsize'],
            nhid=self.params['nhid'],
            nlayers=self.params['nlayers'],
            dropout=self.params['dropout'],
            tie_weights=self.params['tied'],
        )
        local_model.cuda()
        target_model = RNNModel(
            name='Target',
            created_time=self.params['current_time'],
            rnn_type='LSTM',
            ntoken=self.n_tokens,
