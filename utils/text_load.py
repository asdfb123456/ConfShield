...
class Dictionary(object):
    ...
    def add_word(self, word):
        raise ValueError(
            "Please don't call this method, so we won't break the dictionary :) "
        )
...
def get_word_list(line, dictionary):
    splitted_words = json.loads(line.lower()).split()  # 字母全换成小写把单词切分出来
    words = ['<bos>']  # 第一个token
...
class Corpus(object):
    def __init__(self, params, dictionary, is_poison=False):
        ...
        self.train = self.tokenize_train(
            f'{self.path}/shard_by_author', is_poison=is_poison
        )
